...
#   no-mistakes  implement -> /no-mistakes pipeline -> PR -> captain merge (explicit)
...
3. Use Firstmate's hidden-window GitHub bridge for GitHub operations: `$GH_AXI_BASH` from Bash or `& '$GH_AXI_PWSH'` from PowerShell. Use chrome-devtools-axi for browser operations.
...
When it is implemented and committed, push your branch and open a PR with Firstmate's hidden-window GitHub bridge, then append \`done: PR {url}\` to the status file and stop.
