...
- **Explicit project modes** - each project ships via `no-mistakes`, `direct-PR`, or `local-only`, with an optional `+yolo` autonomy flag. This Windows-maintained build defaults new and unregistered projects to `direct-PR`; `no-mistakes` is an explicit choice for critical work.
...
The Windows-maintained build also supports host-assisted `codex-app` tasks using visible Codex Desktop threads.
It prepares Firstmate state without launching a black console window, then uses Desktop `create_thread`, `fork_thread`, `read_thread`, `send_message_to_thread`, and `set_thread_archived` operations for the app-owned lifecycle.
This is not `codex app-server` headless mode.
See [docs/codex-app-backend.md](docs/codex-app-backend.md) for the required two-phase workflow and safety boundary.
...
- [docs/codex-app-backend.md](docs/codex-app-backend.md) - the host-assisted visible Codex Desktop backend workflow and safety contract.
