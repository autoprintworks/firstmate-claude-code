...
Human-authored changes use a normal pull request, focused validation, and explicit maintainer review.
The maintainer runs the `code-review` skill before manually merging.
[`no-mistakes`](https://github.com/kunchenguid/no-mistakes) is optional and is used only when the maintainer explicitly selects it for commercially critical work.

## Workflow

1. Fork the repo, then clone the parent repo or set your local `origin` back to the parent.
2. Create a branch and make your changes.
3. Run the focused tests and lint checks appropriate to the change.
4. Commit and push the branch to your fork.
5. Open a pull request against the parent repository.
6. Address the maintainer's review findings and wait for explicit merge approval.

When the maintainer explicitly selects no-mistakes, follow the installed version's SKILL.md and live `axi` help for that PR instead.
...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  A local `config/backend` file explicitly overrides runtime auto-detection for new task endpoints and stays gitignored; spawn-supported values are `tmux`, host-assisted `codex-app`, plus experimental `herdr`, `zellij`, `orca`, and `cmux`.
  Changes to `codex-app` must prove the visible Desktop lifecycle documented in `docs/codex-app-backend.md`.
  A completed app-server turn is not enough because app-server is headless and does not prove visible project-thread creation, steering, archive, or status return.
  It does not make `data/` tracked.
- Executable entrypoints in `bin/` are plain Bash unless a platform or runtime boundary requires a native adapter.
  Native adapters are limited to the language of that boundary, such as Node.js for the Codex App state bridge, Python for hidden Windows subprocess creation, or PowerShell for a Windows-native entrypoint.
  Each executable starts with an accurate usage header in its language's comment syntax.
...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/` - ship through a direct PR on a feature branch, receive a maintainer `code-review` pass, and require explicit manual merge approval.
...
When no-mistakes is explicitly selected, crewmate validation follows the installed version's SKILL.md and live `axi` help instead of duplicating gate mechanics in firstmate docs.
In that mode, Firstmate's wrapper still matters: crewmates route every `ask-user` finding to firstmate, which applies the authority contract in `AGENTS.md`, and crewmates avoid `--yes` because it would bypass that check and any required captain escalation.
