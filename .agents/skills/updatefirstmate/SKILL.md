...
Firstmate is its own repo.
New tracked material (`AGENTS.md`, `bin/`, `.agents/skills/`, and public `skills/`) reaches a direct PR, the captain runs the `code-review` skill, and the captain manually merges it to `main`; no-mistakes is used only when the captain explicitly selects it for commercially critical work.
The merged material then sits on `main` until each running firstmate pulls it.
