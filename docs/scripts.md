...
| `fm-session-lock-lib.sh` | Shared session-lock harness identity (ancestry walk and holder liveness, with a native arm on Windows) for fm-lock.sh and the Claude Stop auto-arm |
