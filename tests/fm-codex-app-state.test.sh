...
assert_adopt_mode() {
  local id=$1 project=$2 expected_mode=$3 expected_yolo=$4
  FM_ROOT="$TMP" "$ROOT/bin/fm-codex-app" adopt-thread "$id" "thread-$id" "$project" --kind scout >/dev/null
  grep -qx "mode=$expected_mode" "$TMP/state/$id.meta"
  grep -qx "yolo=$expected_yolo" "$TMP/state/$id.meta"
}

rm -f "$TMP/data/projects.md"
assert_adopt_mode missing-registry missing-project direct-PR off

printf '%s\n' '- known [direct-PR] - Known project' > "$TMP/data/projects.md"
assert_adopt_mode unknown-project absent direct-PR off

printf '%s\n' '- legacy - Legacy project' > "$TMP/data/projects.md"
assert_adopt_mode legacy-unlabelled legacy direct-PR off

printf '%s\n' '- invalid [surprise-mode +yolo] - Invalid project' > "$TMP/data/projects.md"
assert_adopt_mode unknown-mode invalid direct-PR off

printf '%s\n' '- newly-created [direct-PR] - Newly created project' > "$TMP/data/projects.md"
assert_adopt_mode new-project newly-created direct-PR off

printf '%s\n' '- critical [no-mistakes] - Explicit critical project' > "$TMP/data/projects.md"
assert_adopt_mode explicit-critical critical no-mistakes off
